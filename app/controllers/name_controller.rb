...
def search
  ...
  @discogs_url = "https://api.discogs.com/database/search?type=release&format=vinyl&release_title=#{@text}&key=#{$discogs_key}&secret=#{$discogs_secret}&per_page=100"
  ...
  $results_array = @unfiltered_results.select do |a_hash|
   a_hash["master_id"] != 0  && a_hash.key?("year") && a_hash["title"] = @text
  end
  ...
  if first_result_hash.nil?
    ...
    if @num_pressings > 1
      ...
      if $num_masters > 1
        redirect_to "/search/name/multreleases/#{@redirect_text}"
      else
        @master_id = @master_ids.uniq.at(0) 
       redirect_to "/search/name/findpressing/#{@master_ids.uniq.at(0)}/#{@redirect_text}"
      end

    #redirectioning if there is only one release
    else
      @record_id = $results_array.at(0).fetch("id")
      redirect_to "/search/name/pressing/#{@record_id}"
    end 
  end 
